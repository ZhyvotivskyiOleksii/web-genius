...
- Створи мінімалістичний, сучасний <header> (шапку сайту).
  ...
  - Реалізуй навігаційне меню з посиланнями-якорями (використовуй надані 'anchors').
...
- Додай ненав'язливий cookie‑consent: маленька смуга внизу (fixed bottom-2 right-2), без перекриття контенту, з однією кнопкою "Accept". Не створюй модалки посеред екрана.
...
- Мова: {{#if language}}{{language}}{{else}}English{{/if}}.
- Підключи TailwindCDN, Font Awesome 6, Material Icons, 1–2 Google Fonts.
- Створи контент для секцій: Use of the Site, Eligibility (18+), Content & Intellectual Property, Disclaimer, Changes, Contact (mailto:contact@{{domain}}).
...
- Заголовок: "{{pageTitle}}". Під ним iframe з грою src="{{gameIframePath}}" (має займати всю доступну висоту/ширину контентної області).
- Явний дисклеймер 18+ поруч або нижче (використовуй іконку або наданий HTML: {{disclaimerHtml}}).
